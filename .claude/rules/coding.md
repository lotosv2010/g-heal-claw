...
- 注释说明"为什么"，而非"做什么"
- 禁止大段注释块，一行足矣
- **禁止在源码注释中嵌入 ADR 编号、任务编号、Phase 编号**（如 `ADR-0016`、`T1.4.1`、`TM.E`、`Phase 3`）——这些属于 `docs/decisions/` 和 `docs/tasks/` 的职责，不应散布在代码中
- 禁止使用"本期"/"下一子任务"/"待 TX.Y 实现"等时态性措辞——注释应描述当前实现状态，而非承诺未来工作

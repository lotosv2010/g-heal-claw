...
function toRow(e: PageViewEvent, geo?: { country: string | null; region: string | null; city: string | null }): NewPageViewRow {
  ...
  return {
    ...
    userId: e.user?.id?.slice(0, 64) ?? null,
    utmSource: e.page?.utm?.source?.slice(0, 128) ?? null,
    utmMedium: e.page?.utm?.medium?.slice(0, 128) ?? null,
    utmCampaign: e.page?.utm?.campaign?.slice(0, 128) ?? null,
    utmTerm: e.page?.utm?.term?.slice(0, 128) ?? null,
    utmContent: e.page?.utm?.content?.slice(0, 128) ?? null,
    searchEngine: e.page?.searchEngine?.slice(0, 32) ?? null,
    channel: e.page?.channel?.slice(0, 64) ?? null,
    ua: e.device?.ua,
    ...
    screenWidth: e.device?.screen?.width ?? null,
    screenHeight: e.device?.screen?.height ?? null,
    screenDpr: e.device?.screen?.dpr ?? null,
    language: e.device?.language?.slice(0, 16) ?? null,
    timezone: e.device?.timezone?.slice(0, 64) ?? null,
    release: e.release,

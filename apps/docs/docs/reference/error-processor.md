...
> Gateway 收到 `type=error` 事件后不再同步落库，而是入 BullMQ 队列 `events-error`，由 `ErrorProcessor` 异步消费并完成 Sourcemap 还原 + 指纹聚合 + Issue UPSERT。
...
| `sync` | Gateway 同步直调 `ErrorsService.saveBatch`（回滚到同步直调行为） | Redis 故障或需紧急回滚时 |
...
本期为 **stub**（`SourcemapService.resolveFrames(events) => events`，原样返回）。完整 source-map v0.7 还原落地后，仅替换 `resolveFrames` 实现体，Processor 无需改动。

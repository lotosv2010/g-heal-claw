...
import type { ExposureTrendBucket } from "@/lib/api/exposure";
...
const METRIC_TABS: readonly MetricTabDef[] = [
  {
    key: "count",
    label: "曝光量",
    subtitle: "每小时 track_events_raw (expose) 入库条数 · 单位：次",
    color: "#8b5cf6",
    seriesLabel: "曝光量",
    yTitle: "次数",
  },
...
export function TrendChart({
  ...
  buckets: readonly ExposureTrendBucket[];
}) {
...
interface SingleChartProps {
  readonly buckets: readonly ExposureTrendBucket[];
  readonly hourFormat: string;
  readonly seriesLabel: string;
  readonly color: string;
  readonly selector: (b: ExposureTrendBucket) => number;
  readonly yTitle: string;
}

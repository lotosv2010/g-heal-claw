...
| **用户留存（Tier 2.E）** | **`/dashboard/v1/tracking/retention`**（ADR-0028；Query：`projectId` 必填 · `cohortDays` 默认 7（1~30）· `returnDays` 默认 7（1~30）· `identity` 默认 `session`（`session`\|`user`）· `since`/`until` ISO 可选；响应：`{source: live\|empty\|error, identity, cohortDays, returnDays, window:{sinceMs,untilMs}, totalNewUsers, averageByDay[]（长度 returnDays+1）, cohorts[{cohortDate, cohortSize, retentionByDay[]}]}` · 比例 4 位小数 · 时间窗不足时返回 source=error 兜底，不 5xx） | GET |
| **数据总览（Tier 3.A）** | **`/dashboard/v1/overview/summary`**（ADR-0029；Query：`projectId` 必填 · `hours` 默认 24（1~168）；响应：`{healthScore, level, domains:{errors,performance,api,resources,visits}}`，每域含 `current/previous/trend/items[]`；`Promise.allSettled` 5 域并发 + 加权公式 errors 40% + LCP 25% + API 20% + resources 15%） | GET |
| **实时监控 SSE（Tier 2.C）** | **`GET /api/v1/stream/realtime`**（ADR-0030；Query：`projectId` 必填 · `topics` CSV 可选（`error,api,perf`，默认全部）· `lastEventId` 可选（断线回放）；协议：`text/event-stream`，`event:` 为 topic 名，`id:` 为 Redis Streams entry ID，`data:` JSON ≤ 256B；15s 心跳；每 projectId 最多 10 并发 SSE 连接（超出 429）；Redis Pub/Sub + Streams MAXLEN ~1000 支持 60s 回放窗口） | SSE |

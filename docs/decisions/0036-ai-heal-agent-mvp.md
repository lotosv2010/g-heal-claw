...
采用「轻量独立进程 + BullMQ 通信 + deepagents ReAct」架构：

1. **apps/ai-agent** — 纯 Node.js 进程（无框架），消费 `ai-diagnosis` 队列，deepagents createDeepAgent 驱动 ReAct 循环
...
| AI 框架 | deepagents + LangChain.js (@langchain/core) | 简洁 Agent API、内置 ReAct + Tool 管理、模型切换 |
...
| 类型 | 路径 |
|---|---|
| AI Agent 源码 | `apps/ai-agent/src/` |
| HealModule 源码 | `apps/server/src/modules/heal/` |
| DDL 迁移 | `apps/server/drizzle/0011_heal_jobs.sql` |
| 仓库配置 Schema | `apps/ai-agent/src/config/repo-config.ts` |
| Heal API 参考文档 | [`apps/docs/docs/reference/heal-api.md`](/reference/heal-api) |
| AI 配置指南 | [`apps/docs/docs/guide/settings/ai.md`](/guide/settings/ai) |
| Demo | 暂不涉及（heal 操作需要真实仓库 + GitHub 凭证） |
